...
void TMR1_IRQHandler(void)
{
    ...
    if(TIMER_GetIntFlag(TIMER1) == 1)
    {
		...
		if ((get_tick() % 100) == 0)
		{
            FLAG_PROJ_TIMER_PERIOD_100MS = 1;
		}	
...
void ReloadPDMA(void)
{
    ...
    PDMA_SetTransferCnt(PDMA, ADC_DMA_CH, PDMA_WIDTH_16, ADCTotalLength);

    /* Set source address as ADC data register (no increment) and destination address as g_iConversionData array (increment) */
    PDMA_SetTransferAddr(PDMA, ADC_DMA_CH, (uint32_t)&ADC->ADPDMA, PDMA_SAR_FIX, (uint32_t)g_iConversionData, PDMA_DAR_INC);

    /* Select PDMA request source as ADC RX */
    PDMA_SetTransferMode(PDMA, ADC_DMA_CH, PDMA_ADC_RX, FALSE, 0);
}
...
void PDMA_IRQHandler(void)
{
    ...
    if (status & PDMA_INTSTS_ABTIF_Msk)   /* abort */
    {
		...
		#else
        if (PDMA_GET_ABORT_STS(PDMA) & (1 << ADC_DMA_CH))
        {
        }
        PDMA_CLR_ABORT_FLAG(, (1 << ADC_DMA_CH));
        if (PDMA_GET_ABORT_STS(PDMA) & (1 << SPI_MASTER_RX_DMA_CH))
        {
        }
        PDMA_CLR_ABORT_FLAG(, (1 << SPI_MASTER_RX_DMA_CH));
		#endif
    }
    else if (status & PDMA_INTSTS_TDIF_Msk)     /* done */
    {
        if((PDMA_GET_TD_STS(PDMA) & ADC_DMA_OPENED_CH) == ADC_DMA_OPENED_CH)
        {
            /* Clear PDMA transfer done interrupt flag */
            PDMA_CLR_TD_FLAG(PDMA, ADC_DMA_OPENED_CH);
    ...
    {
        PDMA_CLR_TMOUT_FLAG(PDMA,ADC_DMA_CH);     
    }
    else
    {
...
{
    PDMA_DisableInt(PDMA, ADC_DMA_CH, PDMA_INT_TRANS_DONE);
...
void PDMA_Init(void)
{
    ...
    /* Open PDMA Channel 4 based on ADC_DMA_CH setting*/
    PDMA_Open(PDMA, ADC_DMA_OPENED_CH);
    ...
    PDMA_SetBurstType(PDMA, ADC_DMA_CH, PDMA_REQ_SINGLE, 0);

    PDMA_EnableInt(PDMA, ADC_DMA_CH, PDMA_INT_TRANS_DONE);
...
uint32_t GetAVDDCodeByADC(void)
{
    ...
    /*
        ...
        ADC conversion rate = 1.5 MHz / 88 = 17.045 ksps
    */
...
uint16_t ADC_InitChannel(uint8_t ch)
{
    ...
    #if 0   // polling
    ADC_Open(ADC, ADC_ADCR_DIFFEN_SINGLE_END, ADC_ADCR_ADMD_SINGLE_CYCLE, 0x1 << ch);
...
{
    ADC_DataArray[ADC0_CH0]     = ADC_InitChannel(ADC0_CH0);
    ADC_DataArray[ADC0_CH1]     = ADC_InitChannel(ADC0_CH1);
    ADC_DataArray[ADC0_CH2]     = ADC_InitChannel(ADC0_CH2);
    ADC_DataArray[ADC0_CH3]     = ADC_InitChannel(ADC0_CH3);
    ADC_DataArray[ADC0_CH4]     = ADC_InitChannel(ADC0_CH4);
    ADC_DataArray[ADC0_CH5]     = ADC_InitChannel(ADC0_CH5);
    ADC_DataArray[ADC0_CH6]     = ADC_InitChannel(ADC0_CH6);  
    ADC_DataArray[ADC0_CH7]     = ADC_InitChannel(ADC0_CH7);
    ADC_DataArray[ADC0_CH8]     = ADC_InitChannel(ADC0_CH8);
    ADC_DataArray[ADC0_CH9]     = ADC_InitChannel(ADC0_CH9);
    ADC_DataArray[ADC0_CH15]    = ADC_InitChannel(ADC0_CH15);
}
...
void ADC_ConvertInit(void)
{
    ...
    /*
        ...
        ADC conversion rate = 1.5 MHz / 17 = 88.235 ksps
    */
...
void regular_log(void)
{
    ...
    {
        res = ADC_To_Voltage(ADC_DataArray[i]);
        printf("%2d:%4dmv[0x%4X],",i , res , ADC_DataArray[i]);
...
void loop(void)
{
    ...
    {
        FLAG_PROJ_TIMER_PERIOD_100MS = 0;   
...
//    CLK_WaitClockReady(CLK_STATUS_LXTSTB_Msk);	
    ...
    SYS->GPB_MFPL = (SYS->GPB_MFPL &~(SYS_GPB_MFPL_PB7MFP_Msk| SYS_GPB_MFPL_PB6MFP_Msk)) \
                    | (SYS_GPB_MFPL_PB7MFP_ADC0_CH7| SYS_GPB_MFPL_PB6MFP_ADC0_CH6) ;
    ...
    GPIO_SetMode(PB, BIT0|BIT1|BIT2|BIT3|BIT4|BIT5|BIT6|BIT7|BIT8|BIT9|BIT15, GPIO_MODE_INPUT);

    /* Disable the PB0 ~ PB3 digital input path to avoid the leakage current. */
    GPIO_DISABLE_DIGITAL_PATH(PB, BIT0|BIT1|BIT2|BIT3|BIT4|BIT5|BIT6|BIT7|BIT8|BIT9|BIT15);
